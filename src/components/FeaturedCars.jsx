    ...
    return(
        <section className='bg-white-100 py-20 sm:px-16 px-4'>
            ...
            <div className='grid gap-6 md:grid-cols-2 lg:grid-cols-3'>
                {cars.map((car) => (
                    <div key={car.id} className='bg-white rounded-xl p-4 shadow-md hover:shadow-lg transition duration-300 
                    hover:-translate-y-3 reveal-y'>
                        <div className='relative overflow-hidden'>
                            <img src={car.image} alt={car.name} className='rounded-md w-full h-48 sm:h-56 md:h-60 object-cover'/>
                            <span className='absolute top-2 left-2 bg-white text-xs font-semibold px-2 py-1 
                            rounded-full shadow'>{car.type}</span>
                            <span className='absolute top-2 right-2 bg-green-500 text-white text-xs font-semibold px-2 py-1 
                            rounded-full shadow'>{car.status}</span>
                        </div>
                        ...
                        <div className='mt-4'>
                            ...
                            <div className='flex flex-wrap gap-2 mt-3'>
                                {car.badges.map((badge, i) => (
                                    <span key={i} className='bg-gray-50 text-xs px-2 py-1 rounded-full font-semibold border 
                                    border-gray-200' >{badge}</span>
                                ))}
                            </div>
                            <div className='mt-4'>
                            ...
                            <div className='flex sm:flex-row flex-col mt-4 gap-3'>
                                <button className='sm:w-1/2 w-full border border-gray-300 px-3 py-2 rounded cursor-pointer 
                                transition duration-300 hover:bg-gray-300'>View Details</button>
                                <button className='sm:w-1/2 w-full border bg-green-600 text-white px-3 py-2 rounded cursor-pointer 
                                transition duration-300 hover:bg-green-700'>Book Now</button>
                            </div>
                        </div>
                    </div>
                ))}
